...
type MockBridge = PlivoCallBridge & {
...
function connectClient(client: PlivoPhoneClient, transport: MockTransport) {
  client.connect();
  transport._fireOpen();
}

async function startCall(client: PlivoPhoneClient, transport: MockTransport) {
...
describe("PlivoPhoneClient", () => {
  let transport: MockTransport;
  let bridge: MockBridge;
  let client: PlivoPhoneClient;

  beforeEach(() => {
    transport = createMockTransport();
    bridge = createMockBridge();
    client = new PlivoPhoneClient({ transport, bridge });
  });
  ...
  describe("connect / disconnect", () => {
    ...
    it("clears error on transport open", () => {
      client.connect();
      transport._fireError();
  ...
  describe("welcome message", () => {
    ...
    it("warns on protocol version mismatch", () => {
      ...
      connectClient(client, transport);
      transport._fireMessage('{"type":"welcome","protocol_version":2}');

      expect(client.serverProtocolVersion).toBe(2);
  ...
  describe("startCall / endCall", () => {
    ...
    it("endCall emits idle statuschange", async () => {
      await startCall(client, transport);
      const handler = vi.fn();
  ...
  describe("audio routing", () => {
    ...
    it("does not send audio when transport is disconnected", async () => {
      await startCall(client, transport);
      transport._fireClose();
      bridge.onAudioData?.(new ArrayBuffer(320));
  ...
  describe("silence / speech detection", () => {
    ...
    it("uses custom silence duration", async () => {
      vi.useFakeTimers();
      client = new PlivoPhoneClient({
        transport,
        bridge,
      ...
      let endCalls = transport.sendJSON.mock.calls.filter(
        (call) => jsonMessageType(call) === "end_of_speech"
      );
      expect(endCalls).toHaveLength(0);

      vi.advanceTimersByTime(500);
      endCalls = transport.sendJSON.mock.calls.filter(
        (call) => jsonMessageType(call) === "end_of_speech"
      );
  ...
  describe("transcript", () => {
    ...
    it("clears playback buffer on user transcript while playing", async () => {
      await startCall(client, transport);
      transport._fireMessage('{"type":"status","status":"speaking"}');
      transport._fireMessage(
        '{"type":"transcript","role":"user","text":"stop"}'
      );
    ...
    it("transcript_start appends empty assistant message", async () => {
      await startCall(client, transport);
      transport._fireMessage('{"type":"transcript_start","role":"assistant"}');

      expect(client.transcript).toHaveLength(1);
      expect(client.transcript[0].role).toBe("assistant");
  ...
  describe("status and metrics", () => {
    ...
    it("tracks error messages from server", async () => {
      await startCall(client, transport);
      const handler = vi.fn();
      client.addEventListener("error", handler);
      ...
      expect(handler).toHaveBeenCalledWith("something broke");
    });
  ...
  describe("messaging", () => {
    ...
    it("sendText is dropped when not connected", () => {
      client.sendText("hello");

      expect(transport.sendJSON).not.toHaveBeenCalled();
    });
    ...
    it("sendJSON is dropped when not connected", () => {
      client.sendJSON({ type: "custom", data: 42 });
